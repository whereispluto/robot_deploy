...
class PolicyCsvTrace:
    """Write one row for every received STM32 state and resulting command."""

    def __init__(self, path: Path) -> None:
        self.path = path
        self._file = path.open("w", encoding="utf-8", newline="", buffering=1)
        self._writer = csv.writer(self._file)
        joint_names = tuple(name.removesuffix("_joint") for name in POLICY_JOINT_NAMES)
        self._writer.writerow(
            ("host_time_ns", "host_elapsed_s", "stm32_time_ms", "command_seq")
            + tuple(f"onnx_raw_{name}" for name in joint_names)
            + tuple(f"action_{name}" for name in joint_names)
            + tuple(f"target_deg_{name}" for name in joint_names)
            + tuple(f"position_deg_{name}" for name in joint_names)
            + tuple(f"velocity_deg_s_{name}" for name in joint_names)
        )
    ...
    def __call__(
        ...
        if self._start_ns is None:
            self._start_ns = host_time_ns
        empty = [""] * POLICY_ACTION_SIZE
        onnx_raw = (
            policy_output.onnx_raw_action
            if policy_output is not None and policy_output.onnx_raw_action is not None
            else empty
        )
        action = policy_output.raw_action if policy_output is not None else empty
        self._writer.writerow(
            [
                host_time_ns,
                f"{(host_time_ns - self._start_ns) * 1.0e-9:.9f}",
                state.timestamp_ms,
                command.seq,
                *onnx_raw,
                *action,
...
def parse_velocity_command(value: str) -> list[float]:
    values = [part.strip() for part in value.split(",")]
    if len(values) != 3:
        raise argparse.ArgumentTypeError("velocity command must contain VX,VY,WZ")
    try:
    ...
    if not all(math.isfinite(item) for item in command):
        raise argparse.ArgumentTypeError("velocity command values must be finite")
    return command
...
class OnnxPolicy:
    ...
    def __init__(
        ...
        self._default_joint_pos = np.asarray(DEFAULT_JOINT_POS_RAD, dtype=np.float32)
        ...
        self._clock = clock
        self._joint_pos_history: deque[np.ndarray] = deque(maxlen=POLICY_HISTORY_LENGTH)
        self._joint_vel_history: deque[np.ndarray] = deque(maxlen=POLICY_HISTORY_LENGTH)
        self._step_count = 0
        ...
        projected_gravity_source = metadata.get("projected_gravity_source", "")
        if projected_gravity_source != "base_link":
            raise RuntimeError(
                "ONNX policy was not trained with base_link projected gravity: "
                f"{projected_gravity_source or 'metadata missing'}"
            )
    ...
    def _append_history(self, history: deque[np.ndarray], value: np.ndarray) -> None:
        if not history:
    ...
    def _startup_output(self, target_joint_pos: np.ndarray) -> PolicyOutput:
        return PolicyOutput(
            command=RobotCommand(
                ...
                kp_nm_per_rad=MOTOR_POSITION_STIFFNESS,
                kd_nms_per_rad=MOTOR_POSITION_DAMPING,
                flags=COMMAND_FLAG_STARTUP_TRAJECTORY,
            ),
            raw_action=[0.0] * POLICY_ACTION_SIZE,
            onnx_raw_action=[0.0] * POLICY_ACTION_SIZE,
        )
    ...
    def _run_startup_sequence(self, joint_pos_deg: np.ndarray) -> PolicyOutput | None:
        ...
        if self._startup_stage == "wait_for_default":
            position_error = self._np.abs(joint_pos_deg - self._default_joint_pos_deg)
    ...
    def __call__(self, state: RobotState, last_action: list[float]) -> PolicyOutput:
        np = self._np
        ...
        if (
            not np.isfinite(joint_pos_deg).all()
            or not np.isfinite(joint_vel_deg_s).all()
        ):
        ...
        projected_gravity = np.asarray(state.gravity_orientation, dtype=np.float32)
        ...
        if any(
            array.shape != (size,)
            for array, size in zip(state_values, (6, 6, 3, 3, 6), strict=True)
        ):
        ...
        gait_phase = np.asarray((math.sin(phase), math.cos(phase)), dtype=np.float32)
        ...
        onnx_raw_action = cast("np.ndarray", raw_output)[0]
        if (
            onnx_raw_action.shape != (POLICY_ACTION_SIZE,)
            or not np.isfinite(onnx_raw_action).all()
        ):
        ...
        raw_action = np.clip(onnx_raw_action, -POLICY_ACTION_CLIP, POLICY_ACTION_CLIP)
        ...
        output = PolicyOutput(
            ...
            onnx_raw_action=[float(value) for value in onnx_raw_action],
        )
...
def make_motion_test_policy() -> Callable[[RobotState, list[float]], RobotCommand]:
    ...
    def policy(state: RobotState, last_action: list[float]) -> RobotCommand:
        ...
        if phase_elapsed < MOTION_TEST_PHASE_S:
            ...
            announce(f"joint {joint_index + 1} -> {target[joint_index]:.3f} deg")
        else:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Linux PC <-> STM32 USB CDC bridge")
    parser.add_argument(
        "--port",
...
def main() -> int:
    ...
    try:
        ...
        print(
            f"Connected to {port} at {args.baudrate} baud; loop rate {args.rate:g} Hz."
        )
        trace: PolicyCsvTrace | None = None
        if args.policy == "onnx":
            trace_dir = Path(__file__).resolve().parent / "log_sim2real"
