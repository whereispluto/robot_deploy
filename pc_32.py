...
class RobotCommand:
    ...
    kp_nm_per_rad: float = 0.1589384924
    kd_nms_per_rad: float = 0.1589384924
    seq: int = 0
    flags: int = 0
...
class PolicyOutput:
    ...
    onnx_raw_action: Optional[list[float]] = None
...
def pack_frame(msg_id: int, payload: bytes, seq: int = 0) -> bytes:
    header = struct.pack(
        FRAME_HEADER_FORMAT, MAGIC, VERSION, msg_id, seq & 0xFFFF, len(payload)
    )
...
def unpack_frame(frame: bytes) -> tuple[int, int, bytes]:
    ...
    magic, version, msg_id, seq, length = struct.unpack_from(
        FRAME_HEADER_FORMAT, frame, 0
    )
    if magic != MAGIC:
...
def unpack_command(payload: bytes) -> RobotCommand:
    ...
    kp_nm_per_rad = float(values[6])
    kd_nms_per_rad = float(values[7])
    seq = int(values[8])
    flags = int(values[9])
    return RobotCommand(
        target_joint_pos=target_joint_pos,
        kp_nm_per_rad=kp_nm_per_rad,
        kd_nms_per_rad=kd_nms_per_rad,
        seq=seq,
        flags=flags,
    )
...
class SerialBridge:
    ...
    def read_frames(self) -> list[bytes]:
        ...
        while True:
            ...
            _, _, _, _, length = struct.unpack_from(
                FRAME_HEADER_FORMAT, self._rx_buffer, 0
            )
...
class RobotBridge:
    def __init__(
        ...
        trace_fn: Optional[
            Callable[[int, RobotState, RobotCommand, Optional[PolicyOutput]], None]
        ] = None,
    ) -> None:
    ...
    def _default_policy(self, state: RobotState) -> RobotCommand:
        target = [
            clamp(value, -self.command_limit, self.command_limit)
            for value in state.joint_pos
        ]
    ...
    def handle_state(self, state: RobotState) -> RobotCommand:
        ...
        if self.trace_fn is not None:
            self.trace_fn(time.perf_counter_ns(), state, command, policy_result)
        return command
...
def main() -> None:
    ...
    parser.add_argument(
        "--port",
        default="COM10",
        help="Serial port, default COM10 (e.g. COM5 or /dev/ttyACM0)",
    )
